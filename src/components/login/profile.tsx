...
export default function AvatarAcc() {
  const [anchorEl, setAnchorEl] = React.useState<null | HTMLElement>(null);
  
  const open = Boolean(anchorEl);
  const {  status,error , mutateAsync } = useLogout()
  const { text } = useNav()
  ...
  return (
    <React.Fragment>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', textAlign: 'center' }}>
        <Typography fontFamily={'unset'}  className={`${text} text-xl `} fontWeight={600} fontSize="1.25rem" sx={{ minWidth: 100 }}>{  `Hi, ${currentUser.name}`}</Typography>
